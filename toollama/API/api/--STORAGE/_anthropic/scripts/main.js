...
export class AltTextGenerator {
    constructor() {
        ...
        if (this.modelSelector) {
            ...
            this.modelSelector.value = "claude-3-5-sonnet-20240620";
            ...
            console.log('Model hard-coded to claude-3-5-sonnet-20240620');
        } else {
    ...
    async makeApiCall(content, base64Image, botId = null) {
        ...
        if (botId === this.ttsBotId) {
            return fetch(`${API_BASE_URL}/alt/anthropic`, {
                method: 'POST',
                headers: {
        ...
        if (base64Image) {
            console.log('Using file upload method for image');
            
            // Force non-streaming mode for more reliable results
            const useStreaming = false;
            console.log('Stream mode setting:', useStreaming ? 'Streaming' : 'Non-streaming');
            ...
            formData.append('model', 'claude-3-5-sonnet-20240620');
            formData.append('max_tokens', 3000);
            
            const promptText = this.currentPrompt?.user || 'Describe this image in detail';
            formData.append('prompt', promptText);
            formData.append('stream', useStreaming ? 'true' : 'false');
            
            // Log what we're sending
            console.log('Sending form data for Anthropic:', {
                model: 'claude-3-5-sonnet-20240620',
                promptLength: promptText.length,
                stream: useStreaming
            });
            
            const url = `${API_BASE_URL}/alt/anthropic`;
            console.log('Request URL:', url);
            
            // Try to use fetch with explicit stream handling
            try {
                ...
                if (!useStreaming) {
                    // For non-streaming responses, try to parse the JSON
                    try {
                        const jsonResponse = await response.json();
                        console.log('Non-streaming response content:', jsonResponse);
                        
                        // Create a mock Response that works like a stream for compatibility
                        const mockStream = new ReadableStream({
                            start(controller) {
                                // Add the alt_text content as raw text to the stream
                                if (jsonResponse && jsonResponse.alt_text) {
                                    controller.enqueue(new TextEncoder().encode(jsonResponse.alt_text));
                                }
                                controller.close();
                            }
                        });
                        
                        // Return a new Response with our mock stream
    ...
    async analyzeImage(base64Content, context) {
        ...
        try {
            ...
            console.log('Starting API call to analyze image...');
            // We're ignoring base64Content and directly passing null (the file is accessed via this.currentFile)
            const response = await this.makeApiCall(null, true); // true signals we have an image to process
            ...
            console.log('Got successful response, starting to process stream...');
            ...
            while (true) {
                ...
                debugCounter++;
                console.log(`Received chunk #${debugCounter} of length:`, chunk.length);
                if (chunk.length < 100) {
                    console.log('Chunk content:', chunk);
                } else {
                    console.log('Chunk content (truncated):', chunk.substring(0, 100) + '...');
                }
                
                // Handle the first content to display container
                if (chunk.trim() && isFirstContent) {
                    document.querySelector('.alt-text-container').style.display = 'block';
                    this.streamingIndicator.style.display = 'none';
                    isFirstContent = false;
                }

                // Special handling for plain text streams without JSON formatting
                if (chunk.trim() && !chunk.includes('{') && !chunk.includes('}')) {
                    console.log('Detected plain text stream, processing directly');
                    fullContent += chunk;
                    this.altTextDisplay.innerHTML = this.processMessageContent(fullContent);
                    const progress = Math.min(90, 50 + (fullContent.length / 20));
                ...
                try {
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            console.log('Line is not valid JSON, treating as raw text:', line);
                            fullContent += line;
                            this.altTextDisplay.innerHTML = this.processMessageContent(fullContent);
                            continue;
                        }
                    ...
                    console.log('Received raw text stream instead of JSON, processing directly');
            ...
            // Final updates
            if (!totalTokens) {
                // If no token count from JSON, estimate
                this.tokenCount = Math.round(fullContent.length / 4);
                this.updateTokenCount();
            }
            this.progressBar.style.width = '100%';
            this.altTextDisplay.innerHTML = this.processMessageContent(fullContent);

            // If after all that we still have no content, try a fallback method
            if (!fullContent.trim()) {
                console.log('No content received from stream, trying fallback non-streaming method');
                try {
                    const fallbackFormData = new FormData();
                    fallbackFormData.append('image', this.currentFile);
                    fallbackFormData.append('model', 'claude-3-5-sonnet-20240620');
                    fallbackFormData.append('prompt', this.currentPrompt?.user || 'Describe this image in detail');
                    fallbackFormData.append('stream', 'false'); // Important: request non-streaming
                    
                    const fallbackResponse = await fetch(`${API_BASE_URL}/alt/anthropic`, {
                        method: 'POST',
                    ...
                    if (fallbackResponse.ok) {
                        ...
                        if (fallbackData.alt_text) {
                            fullContent = fallbackData.alt_text;
                            this.altTextDisplay.innerHTML = this.processMessageContent(fullContent);
                        }
                    } else {
                        console.error('Fallback request failed:', await fallbackResponse.text());
                    }
                } catch (fallbackError) {
            ...
            if (fullContent.length < 100) {
                ...
                console.log('Final content (truncated):', fullContent.substring(0, 100) + '...');
            }
            return fullContent || 'No description available';
        } catch (error) {
    ...
    async handleFile(file) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (this.altTextDisplay) {
                    ...
                    if (this.altTextDisplay.textContent.trim()) {
                        ...
                        copyButton.classList.add('animated-gradient');
                        
                        // Delay showing retry buttons
                        setTimeout(() => {
                            if (this.altTextDisplay.textContent.trim()) {
                                // Show both retry buttons together
                                retryButton.classList.remove('hidden');
                                retryButton.classList.add('animated-gradient');
                                
                                // Show global retry button last
                                setTimeout(() => {
                                    if (this.altTextDisplay.textContent.trim()) {
                                        globalRetryButton.classList.remove('hidden');
                                        globalRetryButton.classList.add('animated-gradient');
                                    }
                                }, 500);
                            }
                        }, 1000);
                ...
                // Only show TTS container if it exists
                const ttsContainer = document.getElementById('ttsContainer');
                if (ttsContainer) {
                    ttsContainer.classList.remove('hidden');
                    
                    // Process TTS if selected and ttsSelector exists
                    const ttsSelector = document.getElementById('ttsSelector');
                    if (ttsSelector && ttsSelector.value !== 'none') {
                        console.log('Processing TTS with voice:', ttsSelector.value);
                        this.ttsHandler.handleTTS(description, ttsSelector.value, this.makeApiCall.bind(this), CAMINA_KEY, API_BASE_URL, CAMINA_TTS);
            ...
            this.showToast('Alt text generated successfully', 'success');

        } catch (error) {
            console.error('File handling error:', error);
            this.messages = [];
            this.altTextDisplay.textContent = 'Error processing image. Please try again.';
            document.querySelector('.alt-text-container').style.display = 'block';
            this.streamingIndicator.style.display = 'none';
            this.retryButton.classList.remove('hidden');
            this.globalRetryButton.classList.remove('hidden');
    ...
    displaySubscriptionStatus(user, memberships) {
        ...
        if (hasActivePaidMembership) {
            ...
            window.location.href = 'https://actuallyusefulai.com/members/anthropic/index.html';
        } else {
