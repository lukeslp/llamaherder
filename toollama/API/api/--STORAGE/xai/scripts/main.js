...
export class AltTextGenerator {
    ...
    async makeApiCall(content, base64Image, botId = null) {
        ...
        // If we have an image, convert it to base64 and send in JSON payload
        if (base64Image) {
            console.log('Using base64 image method for X.AI');
            
            // Create FormData object
            const formData = new FormData();
            formData.append('image', this.currentFile);  // Append the actual file
            formData.append('model', 'grok-2-vision');  // Add model parameter
            formData.append('prompt', this.currentPrompt?.user || 'Generate descriptive alt text for this image');
            formData.append('stream', 'false');
            
            console.log('Sending FormData request with image file');
            
            return fetch(`${API_BASE_URL}/alt/xai`, {
                method: 'POST',
                headers: {
        ...
        const requestBody = {
            model: "grok-2-vision",
            api_key: 'xai-JkOIMXNtbfGaTf8iXQUhSPt1LZxwiMbMVWMI4mKQr8wkStYII7BgsYBiQcZ5KZNSn0OPEbxsnM61WkEk',
            messages: [
    ...
    async analyzeImage(base64Content, context) {
        ...
        try {
            ...
            if (contentType && contentType.includes('application/json')) {
                console.log('Received JSON response, processing directly');
                const jsonData = await response.json();
                if (jsonData.alt_text) {
                    this.altTextDisplay.innerHTML = this.processMessageContent(jsonData.alt_text);
                    this.progressBar.style.width = '100%';
                    return jsonData.alt_text;
                }
            }

            // If not JSON, process as stream
    ...
    displaySubscriptionStatus(user, memberships) {
        ...
        if (hasActivePaidMembership) {
            ...
            window.location.href = 'https://actuallyusefulai.com/members/xai/index.html';
        } else {
