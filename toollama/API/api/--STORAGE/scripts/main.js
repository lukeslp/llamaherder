...
export class AltTextGenerator {
    ...
    async analyzeImage(base64Content, context) {
        ...
        try {
            ...
            const response = await this.makeApiCall(null, true);
            ...
            console.log('Got successful response, processing...');
            ...
            const altText = jsonResponse.alt_text;
            this.altTextDisplay.innerHTML = this.processMessageContent(altText);
            document.querySelector('.alt-text-container').style.display = 'block';
            this.streamingIndicator.style.display = 'none';
            this.progressBar.style.width = '100%';

            // Estimate token count since we don't get it directly
            this.tokenCount = Math.round(altText.length / 4);
            this.updateTokenCount();

            console.log('Image analysis complete');
            return altText || 'No description available';
        } catch (error) {
